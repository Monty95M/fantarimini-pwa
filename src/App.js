...
export default function App() {
  ...
  const handleAddPoints = () => {
    ...
    setSelectedPlayer(""); setSelectedEvent("");
  };

  const handleManualChange = (name, newVal) => {
    const val = parseFloat(newVal);
    if (isNaN(val)) return;
    setHistory(prev => [...prev, players]);
    setPlayers(players.map(p =>
      p.name === name ? { ...p, points: val } : p
    ));
  };
  ...
  const handleReset = () => {
    if (!window.confirm("Azzero la classifica?")) return;
    setHistory(prev => [...prev, players]);
    ...
    <div style={{ padding: 16, maxWidth: 400, margin: "auto", fontFamily: "Arial" }}>
            ...
            {sorted.map((p, i) => (
              <div key={p.name} style={{ display:"flex", justifyContent:"space-between", marginBottom: 8 }}>
                ...
                <input
                  ...
                  step="0.1"
                  value={p.points}
                  onChange={e => handleManualChange(p.name, e.target.value)}
                  style={{ width: 60, textAlign: "right" }}
                />
              </div>
            ))}
          </div>
          <button onClick={handleReset} style={{ marginTop: 16, background: "#dc3545", color: "white", padding: 10, border: "none", borderRadius: 6 }}>
            🔄 Azzera Classifica
          </button>
        </>
      )}
      ...
      {tab === "registra" && (
        <div style={{ border: "1px solid #ccc", borderRadius: 8, padding: 16 }}>
          ...
          <button onClick={handleAddPoints} style={{ width:"100%", padding:10, background:"#007bff", color:"white", border:"none", borderRadius:6 }}>
            ✅ Aggiungi Punti
          </button>
          <button onClick={handleUndo} disabled={history.length===0} style={{ marginTop: 12, width:"100%", padding:10, background:"#6c757d", color:"white", border:"none", borderRadius:6 }}>
